...
import {
  startOfWeekMonday, addDays, toISODate,
  ROWS, RowKey, PROCS, ProcKey
} from '@/lib/date';
import {
  listWeek, addItem, updateItem, deleteItem, subscribeItems,
  getMyRole, Item, getMaxOrd
} from '@/lib/data';
import {
...
function procColor(proc: ProcKey): string {
  ...
  if (['oclusión crónica','oclusion crónica','oclusión cronica','oclusion cronica'].includes(p))
  ...
  return (
    <div className="border rounded-lg bg-white p-6">
      ...
      <div className="text-sm text-gray-600">
        Introduce tu email y contraseña en la parte superior. Si no recuerdas la contraseña,
        usa “¿Olvidaste la contraseña?”.
      </div>
    </div>
  );
}
...
function AuthButtons() {
  ...
  const doLoginPassword = async () => {
    try {
      ...
      window.location.replace('/');   // fuerza re-evaluación de rol y estado
    } catch (e) {
      showErr(e);
  ...
  const doLogout = async () => {
    try {
      setBusy(true);
      await supabase.auth.signOut();  // limpia sesión local + server
  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      <button
        ...
        onClick={doLoginPassword} disabled={busy} title="Entrar con email y contraseña">
        Entrar
      </button>
      ...
      <button
        className="px-3 py-1 rounded border bg-white text-sm disabled:opacity-60"
        onClick={doLogout} disabled={busy} title="Cerrar sesión">
        Salir
      </button>
    </div>
  );
    ...
    <div className="bg-white rounded-xl border p-3 flex flex-col gap-3 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button className="p-1 rounded hover:bg-gray-100" title="Guardar"
...
export default function PageClient() {
  ...
  useEffect(() => { const u = subscribeItems(reload); return () => u(); }, [reload]);
      ...
      for (const row of ROWS) {
        ...
        cell.forEach((it, idx) => {
          rows.push([
            ...
            it.name ?? '', it.room ?? '', it.dx ?? '',
            it.proc ?? '', it.done ? 'Sí' : 'No'
          ].map(escapeCSV).join(','));
        });
      }
    }
  ...
  // reorden 1 posición (parking entero)
  const moveOneUp = async (it: Item) => {
    ...
    const prev = cell[idx-1]; const PARK = (cell[0]?.ord ?? 0) - 100000;
    try {
  ...
  const moveOneDown = async (it: Item) => {
    const cell = items.filter(i => i.day === it.day && i.row === it.row).sort((a,b)=>a.ord-b.ord);
    const idx = cell.findIndex(i => i.id === it.id); if (idx >= cell.length-1) return;
    const next = cell[idx+1]; const PARK = (cell[cell.length-1]?.ord ?? 0) + 100000;
    try {
      await updateItem(it.id,   { ord: PARK });
      ...
      {role === 'unknown' ? (
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <input className="ml-auto border rounded px-2 py-1"
                   placeholder="Buscar…" value={search}
                   onChange={(e) => setSearch(e.target.value)} />
            <button className="px-2 py-1 border rounded bg-white flex items-center gap-1"
                    onClick={exportCSV} title="Exportar semana en CSV">
...
/* ============ subcomponentes grandes (fila/celda/tarjeta) ============ */
        ...
        return (
          <div key={dk + row}
            ...
            <div className="flex flex-col gap-2">
              ...
              {canEdit && isDraftHere && (
                <InlineEditorCard title="Nuevo paciente" onCancel={onCancelAdd}
